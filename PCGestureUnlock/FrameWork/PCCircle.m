...
- (instancetype)init
{
    if (self = [super init]) {
        ...
        self.layer.cornerRadius = CircleRadius;//有更改
        self.layer.masksToBounds = YES;//有更改
...
- (void)drawRect:(CGRect)rect {
    ...
    //画边框
    [self drawBorderCircleWithContext:ctx rect:rect radio:radio + 0.2 color:self.inCircleColor];
 ...
 *  @param color 绘制颜色
 */
- (void)drawBorderCircleWithContext:(CGContextRef)ctx rect:(CGRect)rect radio:(CGFloat)radio color:(UIColor *)color
{
    UIColor *bodercolor = color;
    if (bodercolor == CircleStateSelectedInsideColor) {
        bodercolor = [UIColor greenColor];
    }
